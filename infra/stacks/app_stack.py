...
class AppStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, *,
                 runtime: PersistentRuntimeStack, dns: PersistentDnsStack,
                 **kwargs) -> None:
        ...
        self.alb_sg = ec2.SecurityGroup(
            ...
            description="ALB: public HTTPS/HTTP in", allow_all_outbound=True,
        )
        self.alb_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(443), "HTTPS from internet")
        self.alb_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(80), "HTTP (redirect to 443)")

        self.ui_sg = ec2.SecurityGroup(
            self, "UiSg", vpc=self.vpc,
            description="UI task: only from ALB", allow_all_outbound=True,
        )
        self.ui_sg.add_ingress_rule(self.alb_sg, ec2.Port.tcp(8501), "Streamlit from ALB only")

        self.api_sg = ec2.SecurityGroup(
            self, "ApiSg", vpc=self.vpc,
            description="API task: only from UI, NO public ingress", allow_all_outbound=True,
        )
        self.api_sg.add_ingress_rule(self.ui_sg, ec2.Port.tcp(8000), "FastAPI from UI only")
        ...
        self.namespace = servicediscovery.HttpNamespace(
            self, "ServiceConnectNs", name="local",
        )
        ...
        api_task_role.add_to_policy(iam.PolicyStatement(
            ...
            resources=[f"arn:aws:bedrock:{self.region}::foundation-model/{TITAN_MODEL_ID}"],
        ))
        ...
        api_container = self.api_task.add_container(
            ...
            secrets={
                ...
                "qdrant_api_key": ecs.Secret.from_secrets_manager(runtime.qdrant_api_key),
                "anthropic_api_key": ecs.Secret.from_secrets_manager(runtime.anthropic_api_key),
        ...
        ui_container = self.ui_task.add_container(
            "ui",
            image=image,
            command=["streamlit", "run", "app/ui/home.py",
        ...
        ui_container.add_port_mappings(
            ecs.PortMapping(name="ui", container_port=8501)
        )
        ...
        # ---------- UI service (client of api.local; ALB target added in Step 7) ----------
        ...
        https = self.alb.add_listener(
            "Https",
            port=443,
            certificates=[elbv2.ListenerCertificate.from_arn(dns.certificate.certificate_arn)],
            open=False,  # alb_sg already allows 443/80; don't add duplicate 0.0.0.0/0 rules
        )
        https.add_targets(
            "UiTarget",
            port=8501,
            protocol=elbv2.ApplicationProtocol.HTTP,
            targets=[self.ui_service.load_balancer_target(
                container_name="ui", container_port=8501,
            )],
            health_check=elbv2.HealthCheck(
                path="/_stcore/health", healthy_http_codes="200",
            ),
        )
