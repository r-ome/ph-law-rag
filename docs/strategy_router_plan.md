...
**R1 addendum (2026-07-07, runs `intent_ab_20260707_212231` / `intent_ab_20260707_222634`):** two challenge arms ran on the same frozen prompt and benchmark — **Haiku keeps the seat**. `qwen3:4b` scored 69.1% routed with amendment precision 1.00 (zero false `current_law` fires, inside the 2–3-row window) but is disqualified on latency: median 21s/query (mean 25.7s) against the ~1–2s budget the gate assumed. Not thinking tokens (`think: False` confirmed in payload and outputs — clean 43–60-char JSON, zero think tags) and not contention from the concurrent gemma3 pull: fresh contention-free probes still ran 19–33s. The cost is prompt prefill — qwen3:4b is intrinsically ~12× slower than gemma3:4b on the identical prompt on this hardware. `gemma3:4b` is fast (1.8s median) but unsafe in the only lane that changes behavior: 3 false `current_law` activations (watch-row 42 online-libel plus two OOS rows routed into the amendment lane), OOS recall 0.25, and it **misses eval_051** — the preset's sole registered-evidence row — routing it to the demoted `citation_lookup`. No confidence gate can rescue it: gemma3 self-reports `high` on every `current_law` decision (true fires, false fires, and misses alike), and constraint 2 forbids iterating the prompt against the benchmark. A gemma3-local / Haiku-cloud hybrid was considered and rejected — on this evidence gemma3-local is strictly worse than no router. **Decision for R4: Haiku router on serving surfaces; the local path runs routerless** (router-off is byte-identical to today by R2 construction, so local loses nothing). Measured latencies (median, fresh calls): Haiku 1.2s, mistral 0.9s, gemma3 1.8s, qwen3 21.0s, NLI 0.1s warm.
