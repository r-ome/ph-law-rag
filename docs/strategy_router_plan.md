...
**R1 second addendum (2026-07-08, run `intent_ab_gemma4e4b`):** `gemma4:e4b` (post-benchmark model release, 9.6 GB blob) ran the same frozen prompt/benchmark through a standalone runner with structural thinking verification — direct `/api/chat` (bypassing `llm_client`'s `</think>` strip, the layer that hid qwen3's thinking), per-call `eval_count` logged, alarm at >50 tokens. **Thinking verified genuinely off: 0/81 alarms, max 20 tokens/call — `think: false` works on gemma4 where Ollama 0.31.1 silently ignores it for qwen3.** Results: routed 64.2% (below gemma3 — one more datapoint that routed/intent-level accuracy measures the wrong thing), **strategy-level 92.6% (75/81, ties qwen3-thinking for board best)**, amendment 8/13 with precision 0.89 (one false fire), routes eval_051 correctly, healthy confidence gate (7 low-confidence rows correctly fell back to `default` — no gemma3-style degenerate always-`high`), OOS recall 0.58, 1.0s median in isolation. The tie with Haiku (75 vs 74 = one question) breaks on the other columns: local, $0, best-in-class amendment behavior. **Two blockers keep the seat decision unchanged for now.** (1) **Contingent, not settled, harm verdict:** the single false fire (row 12, Family Code below-18 marriage) is a guaranteed no-op *today* only because `provision_supersession.yaml` has no Family Code pairs — the scheduled supersession-map coverage audit could make exactly that entry land, flipping the fire from no-op to live behavioral error. Re-run the harm trace (gemma4's row 12 AND gemma3's 3 rows) against the filled map as part of the map-fill task; do not file as settled. (2) **Serving residency failure — an upstream memory-estimation bug, not a capacity problem.** Reproducibly, both directions, loading gemma4:e4b evicts mistral+nomic-embed and vice versa; gemma4-specific (gemma3:4b coexists with both at 100% GPU). Actual usage is ~3.1–3.3 GB resident — all three models total under 9 GB actual on an 18 GB machine — but Ollama 0.31.1's *predicted* memory for the Gemma 4 MoE architecture demands exclusive residency. Both cheap knobs tested and dead: `num_ctx: 2048` no effect (not KV-driven), and the 6.1 GB `e4b-it-qat` variant (3.1 GB resident) evicts identically (not blob-size-driven; note the default `e4b` blob IS `q4_K_M` — same digest — so there is no smaller mainline quant). The estimate is a property of the architecture in the scheduler, exactly the kind of thing point releases fix (v0.31.2-rc2, 2026-07-06, bumps llama.cpp + MLX engine; no explicit fix named). **Tripwire, not a task: on each Ollama release, re-run the 5-minute `ollama ps` coexistence test** (load mistral + nomic-embed, then gemma4:e4b, check all three stay resident). Until it passes, a routed query under local serving load pays ~8–9s router reload plus the generator's reload on the next call — the benchmark's 1.0s exists only in isolation. Upstream issue draft prepared (no matching issue exists in ollama/ollama as of 2026-07-08). **Standing decision: Haiku on serving surfaces; local path routerless. gemma4:e4b retires gemma3 as the designated local arm on quality, blocked from local deployment only by the upstream estimate bug (no bigger machine needed); gemma3 remains the only local router that currently coexists with the serving stack.** Per the R5 endgame: if index-time amendment handling moots the `current_law` lane, this seat contest retires unnecessary — no further coronation investment before the map audit and R5.
